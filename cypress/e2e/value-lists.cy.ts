['localStorage', 'indexedDB'].forEach(storageType => {
  describe(`Value Lists - ${storageType}`, () => {
    beforeEach(() => {
      cy.clearAllStorage();
    ...
    describe('Variable Detection', () => {
      it('should show detected variables from templates', () => {
        ...
        cy.get('.list-block').should('have.length', 2);
        cy.get('.variable-label').eq(0).should('contain.text', 'name');
        cy.get('.variable-label').eq(1).should('contain.text', 'status');
      });
      ...
      it('should show empty state when no templates have variables', () => {
        ...
        cy.get('.empty-state').should('contain.text', 'No template variables detected');
      });
      ...
      it('should show "no options" for variables without values', () => {
        ...
        cy.get('.no-values').should('contain.text', 'no options');
      });
    });
    ...
    describe('Adding Values to Variables', () => {
      ...
      it('should expand a variable to add values', () => {
        cy.get('.list-block').first().click();
        cy.get('.sliding-panel.visible .values-section').should('be.visible');
        cy.get('.empty-values').should('contain.text', 'No options yet');
      });

      it('should add values to a variable', () => {
        // Click on status variable
        cy.get('.variable-label').contains('status').click();
      ...
      it('should show count badge after adding values', () => {
        ...
        cy.get('.list-block').first().find('.count-badge').should('contain.text', '2');
      });
    ...
    describe('Persistence', () => {
      it('should persist value lists across page reloads', () => {
        ...
        openValueListManager();
        cy.get('.list-block').should('have.length', 1);
        cy.get('.count-badge').should('contain.text', '2');
